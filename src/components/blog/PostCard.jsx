        ...
        <Link to={href} className="block group">
          <div className="liquid-glass rounded-2xl p-6 h-full transition-all duration-300 hover:border-white/30 hover:shadow-[0_12px_40px_rgba(31,38,135,0.25)]">
            ...
            <div className="flex items-center justify-between mb-3">
              <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${post.categoryColor || 'bg-white/30 text-[var(--text-muted)]'}`}>
                {post.categoryLabel}
              </span>
            ...
            {/* Title */}
            <h2 className="text-base font-bold text-[var(--text-heading)] mb-2 line-clamp-2 group-hover:gradient-text transition-all duration-200 leading-snug">
              {post.title}
            </h2>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-white/20 mt-auto">
              ...
              <div className="flex items-center gap-1 text-xs font-medium text-[var(--color-primary)] opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <span>阅读</span>
                <ArrowRight size={11} />
              </div>
            </div>
