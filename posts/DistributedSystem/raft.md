...
# Raft 算法详解（上）：从设计哲学到领导者选举与日志复制

## 一、引言：为什么需要 Raft？

在分布式系统中，数据一致性是一个核心挑战。当多台服务器组成集群协同工作时，如何保证它们对同一份数据达成一致的认知？这个问题在学术界被称为**共识问题**（Consensus Problem）。

在 Raft 出现之前，Paxos 算法一直是分布式共识领域的“事实标准”。然而，Paxos 以其极高的复杂度著称——它的“单决策”与“多决策”分层设计、复杂的角色转换逻辑，让大多数开发者望而却步。正如 Raft 论文作者所言，Paxos 虽然理论正确性已被充分验证，但工程化门槛极高，基于 Paxos 的成熟工程实践非常稀少。

2013 年，Diego Ongaro 和 John Ousterhout 发表了 Raft 算法，其核心目标简洁而有力：**设计一个易于理解和实现的共识算法**。Raft 通过**模块化设计**将复杂的共识问题分解为三个相对独立的子问题：

1. **领导者选举**（Leader Election）：如何高效、安全地选出一个唯一的领导者；
2. **日志复制**（Log Replication）：如何确保领导者与跟随者的日志保持一致；
3. **安全性**（Safety）：如何防止脑裂、数据回退等异常场景。
...
要理解 Raft，首先需要理解它要解决什么问题。

共识算法通常出现在**复制状态机**（Replicated State Machine）的背景下。简单来说，一组服务器各自维护一个状态机（比如一个哈希表）和一个日志。状态机从日志中按顺序读取指令并执行。共识算法的任务就是保证**所有服务器日志中的指令以相同的顺序出现**——如果服务器 A 的第 n 条指令是 `set x = 3`，那么任何其他服务器的第 n 条指令也必须是 `set x = 3`。

这样一来，即使少数服务器发生故障，整个集群对外看起来仍然像一个可靠的、单一的状态机。Raft 的核心任务，就是管理这个复制日志。

### 2.2 强领导者模型

Raft 采用了一种 **“强领导者”** 的设计哲学。与一些对称式的共识算法不同，Raft 将一切决策权集中到领导者手中：
...
| 角色                    | 职责                                                   |
| ----------------------- | ------------------------------------------------------ |
| **领导者（Leader）**    | 处理所有客户端请求，管理日志复制，定期发送心跳维持权威 |
| **跟随者（Follower）**  | 被动接收并复制领导者的日志，响应投票请求               |
| **候选者（Candidate）** | 临时角色，用于在选举中竞争成为领导者                   |

节点启动时默认都是跟随者状态。这三种角色之间的转换由 Raft 算法严格规定，下一节将详细展开。

### 2.4 任期（Term）

Raft 将时间划分为一个个 **“任期”** （Term）。每个任期都以一次选举开始：

- 任期号是一个**连续递增的整数**；
- 每个任期内最多只有一个领导者；
- 任期号是节点判断信息新旧的重要依据——如果某个节点发现自己的任期号比其他节点小，会立即将自己的任期号更新为更大的值；
- 如果某个节点收到任期号比自己小的请求，会**拒绝**该请求。

任期机制是 Raft 保证安全性的基石之一，它让集群能够区分“过期”的信息和“当前”的信息。
